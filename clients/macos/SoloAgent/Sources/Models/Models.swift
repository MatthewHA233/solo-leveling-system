...
struct AgentReport: Codable {
    ...
    func toJSON() -> [String: Any] {
        return [
            ...
            "snapshot": [
                "screenshot_b64": snapshot.screenshotBase64 ?? "",
                "active_window": [
                    "app_name": snapshot.activeWindow.appName ?? "",
                    "window_title": snapshot.activeWindow.windowTitle ?? "",
                    "bundle_id": snapshot.activeWindow.bundleId ?? ""
                ],
                "idle_seconds": snapshot.idleSeconds,
                "is_screen_locked": snapshot.isScreenLocked
            ]
        ]
    }
}
...
struct Snapshot: Codable {
    ...
    var screenshotBase64: String? {
        screenshotData?.base64EncodedString()
    }
