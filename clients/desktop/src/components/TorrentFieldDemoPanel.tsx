// ══════════════════════════════════════════════
// TorrentFieldDemoPanel — 洪流域早期静态示范（保留参考用）
//   保留 HudFrame + 鱼骨 timeline + 锚点提取 + ThoughtDock 的原貌，
//   作为"洪流域(静态示范)" tab 长期展示，方便对照之后的真实实现迭代。
//   不再随主线演进，所有交互/数据都是 mock，不接 DB。
// ══════════════════════════════════════════════
...
const THOUGHT_STORAGE_KEY = 'sls.torrent.thoughtCards.v1'
...
export default function TorrentFieldDemoPanel() {
  const [filter, setFilter] = useState<SourceFilter>('all')
  const [hoveredId, setHoveredId] = useState<string | null>(null)
  const [selectedId, setSelectedId] = useState(sampleEvents[0].id)
  const [thoughtMode, setThoughtMode] = useState<ThoughtMode>('plain')
  ...
  const submitThought = () => {
    ...
    const nextCard: ThoughtCard = {
      ...
      ...(thoughtMode === 'context' ? { contextLabel: '语境接口预留' } : {}),
    }
      ...
      <main style={styles.content}>
        <header style={styles.header}>
          ...
          <div style={styles.metrics}>
            <Metric label="想法" value={thoughtCards.length} color={theme.textPrimary} />
...
function ThoughtDock({
  ...
  readonly onModeChange: (mode: ThoughtMode) => void
  readonly onDraftChange: (text: string) => void
  readonly onTopicChange: (text: string) => void
  readonly onSubmit: () => void
  readonly onRemove: (id: string) => void
}) {
  return (
    <section style={styles.thoughtDock}>
      <div style={styles.thoughtComposer}>
        <div style={styles.thoughtModeRow}>
          <ModeButton label="纯想法" active={mode === 'plain'} onClick={() => onModeChange('plain')} />
          <ModeButton label="带语境" active={mode === 'context'} onClick={() => onModeChange('context')} />
          {mode === 'context' && <span style={styles.contextStub}>暂时只保留接口，后续再接 B站 / 窗口 / 网页语境</span>}
        </div>
        <textarea
          value={draft}
          ...
          placeholder={mode === 'plain' ? '像 flomo 一样，先把此刻冒出来的一句话记下来。' : '先写想法；语境锚点暂时用接口占位。'}
          style={styles.thoughtInput}
        />
        <div style={styles.thoughtComposerFooter}>
          <input
            value={topic}
      ...
      <div style={styles.thoughtFeed}>
        {cards.length === 0 ? (
          <div style={styles.thoughtEmpty}>还没有想法。这里先做低压入口：写一句，留下来。</div>
        ) : (
          cards.slice(0, 6).map((card) => (
            <article key={card.id} style={styles.thoughtCard}>
              <div style={styles.thoughtCardMeta}>
                <span>{timeLabel(card.createdAt)}</span>
                {card.topic && <b>{card.topic}</b>}
                {card.mode === 'context' && <i>{card.contextLabel ?? '带语境'}</i>}
                <button type="button" onClick={() => onRemove(card.id)} style={styles.thoughtDelete}>×</button>
              </div>
              <p style={styles.thoughtText}>{card.text}</p>
            </article>
          ))
        )}
      </div>
...
function ModeButton({ label, active, onClick }: {
  ...
  return (
    <button
      ...
      style={{
        ...styles.modeButton,
        color: active ? theme.background : theme.flameTeal,
        background: active ? theme.flameTeal : 'rgba(0,255,224,0.06)',
        borderColor: active ? theme.flameTeal : `${theme.flameTeal}55`,
      }}
    >
      {label}
    </button>
...
const styles: Record<string, CSSProperties> = {
  ...
  thoughtDock: {
    ...
    minHeight: 172,
    display: 'grid',
    gridTemplateColumns: 'minmax(360px, 0.76fr) minmax(360px, 1fr)',
    gap: 12,
  },
  thoughtComposer: {
    minHeight: 0,
    ...
    background: 'linear-gradient(180deg, rgba(0,255,224,0.075), rgba(0,0,0,0.24))',
  ...
  thoughtSubmit: {
    height: 28,
    border: `1px solid ${theme.flameTeal}`,
  ...
  thoughtCard: {
    ...
    background: 'linear-gradient(180deg, rgba(255,153,51,0.075), rgba(0,0,0,0.28))',
    clipPath: hud.chamfer12,
    display: 'flex',
    flexDirection: 'column',
    gap: 9,
  ...
  thoughtCardMeta: {
    display: 'grid',
    gridTemplateColumns: 'auto minmax(0, 1fr) auto 18px',
    gap: 7,
    alignItems: 'center',
    color: theme.textMuted,
    fontFamily: theme.fontMono,
