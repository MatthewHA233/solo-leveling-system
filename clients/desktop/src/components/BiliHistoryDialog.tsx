...
const SEARCH_PAGE = 40         // 搜索结果一页条数（下滑追加下一页）
...
export default function BiliHistoryDialog({
  ...
  const [searchLoading, setSearchLoading] = useState(false)   // 首页加载
  const [searchLoadingMore, setSearchLoadingMore] = useState(false) // 下滑追加
  const [searchHasMore, setSearchHasMore] = useState(false)
  const searchLoadMoreRef = useRef<() => void>(() => {})       // 滚动里调，避免闭包陈旧
  ...
  // 下载完成 / 转录完成 / 删除 → 刷新当日列表（让卡片绿框 + chip 数字立即更新）
  // 同时清掉日历的月度计数缓存，下次打开 DatePicker 会重拉
  useEffect(() => {
    if (!open) return
    const refresh = () => { invalidateBiliMonthCache(); loadSpans(date) }
    // 下载阶段事件（后端 tauri emit）：完成时刷新
    const u = listen<{ stage: string }>('bili-download-progress', (e) => {
      if (e.payload.stage === 'done') refresh()
    })
    // B站资产变更（前端 window 事件）：转录完成 / 删除下载时刷新
    const onAssetsChanged = () => refresh()
    window.addEventListener('solevup:bili-assets-changed', onAssetsChanged)
    return () => {
      u.then((fn) => fn()).catch(() => {})
      window.removeEventListener('solevup:bili-assets-changed', onAssetsChanged)
    }
  }, [open, date, loadSpans])
  ...
  // 搜索 debounce：query 变 → 重置并加载首页（一页 SEARCH_PAGE 条）
  useEffect(() => {
    if (!searchOpen) return
    const q = searchQuery.trim()
    if (!q) { setSearchResults([]); setSearchLoading(false); setSearchHasMore(false); return }
    setSearchLoading(true)
    let cancelled = false
    const t = setTimeout(() => {
      searchBiliHistory(q, SEARCH_PAGE, 0)
        .then((rows) => {
          if (cancelled) return
          setSearchResults(rows)
          setSearchHasMore(rows.length >= SEARCH_PAGE) // 满页 → 可能还有，下滑再取
        })
        .catch(() => { if (!cancelled) { setSearchResults([]); setSearchHasMore(false) } })
        .finally(() => { if (!cancelled) setSearchLoading(false) })
    }, 220)
    return () => { cancelled = true; clearTimeout(t) }
  }, [searchQuery, searchOpen])

  // 下滑加载更多：以当前结果数为 offset 取下一页追加。用 ref 暴露给滚动回调，规避闭包陈旧
  searchLoadMoreRef.current = () => {
    const q = searchQuery.trim()
    if (!q || searchLoading || searchLoadingMore || !searchHasMore) return
    setSearchLoadingMore(true)
    searchBiliHistory(q, SEARCH_PAGE, searchResults.length)
      .then((rows) => {
        // 按 bvid 去重后追加（防同 view_at 边界重复）
        setSearchResults((prev) => {
          const seen = new Set(prev.map((r) => r.bvid))
          return [...prev, ...rows.filter((r) => !seen.has(r.bvid))]
        })
          ...
          >
            ...
            <div
              ...
              onScroll={(e) => {
                ...
                if (el.scrollHeight - el.scrollTop - el.clientHeight < 80) searchLoadMoreRef.current()
              }}
            >
              {!searchQuery.trim() ? (
                ...
                <>
                  ...
                  {(searchLoadingMore || searchHasMore) && (
                    <div style={{ padding: '8px 12px', textAlign: 'center', color: theme.textMuted, fontSize: 10, letterSpacing: 0.3 }}>
                      {searchLoadingMore ? '加载更多…' : '下滑加载更多'}
                    </div>
                  )}
                </>
              )}
            ...
            <div style={{
              ...
              <span>{searchResults.length > 0 ? `${searchResults.length} 条${searchHasMore ? '+' : ''}` : ''}</span>
...
function DetailOverlay({
  ...
  // 该视频被删除：收起影院 + 关闭转录 + 清空本地路径（避免残留指向已删文件）
  useEffect(() => {
