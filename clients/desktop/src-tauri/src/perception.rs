...
// 浏览器先按 ManicTime 的低侵入路线处理：读取前台窗口标题/进程，不依赖扩展。
// ══════════════════════════════════════════════
                        ...
                        if let Some(exe_path) = snapshot.exe_path.as_deref() {
                            match extract_app_icon_bmp(exe_path) {
                                Some(bytes) => {
                                    ...
                                    if let Err(err) = db.update_perception_app_icon(&snapshot.app, Some(exe_path), &bytes, dominant.as_deref()).await {
                                        log::warn!("[Perception] app icon save failed: {}", err);
                                    }
                                }
                                None => log::debug!("[Perception] no icon extracted for {}", exe_path),
                            }
                        }
                    }
...
pub fn clear_screenshot_data() -> Result<ScreenshotStorageInfo, String> {
    ...
    let root = PathBuf::from(&settings.save_dir);
    fs::create_dir_all(&root).map_err(|e| format!("创建截图目录失败: {}", e))?;
    let root = root.canonicalize().map_err(|e| format!("解析截图目录失败: {}", e))?;
...
fn capture_screenshot_now(settings: &ScreenshotSettings) -> Result<PathBuf, String> {
    ...
    let (x, y, width, height) = capture_rect(settings)
        .ok_or_else(|| "截图区域尺寸无效".to_string())?;
...
pub fn add_window_blacklist(app: String, title: Option<String>) -> Result<Vec<WindowBlacklistEntry>, String> {
    ...
    let dup = list.iter().any(|e| e.app.eq_ignore_ascii_case(&app) && e.title == title);
    if !dup {
...
pub fn remove_window_blacklist(app: String, title: Option<String>) -> Result<Vec<WindowBlacklistEntry>, String> {
    let title = title.map(|t| t.trim().to_string()).filter(|t| !t.is_empty());
    let mut list = load_window_blacklist();
...
pub async fn refresh_app_colors_from_icons(db: Arc<Database>) {
    ...
    log::info!("[Perception] 主色刷新：扫 {} / 更新 {}", entries.len(), updated);
}
...
fn dominant_color_from_bmp(bytes: &[u8]) -> Option<String> {
    ...
    let pass = |sat_min: i32, lum_min: u32, lum_max: u32| -> Option<(u8, u8, u8)> {
        ...
        for px in pixels.chunks_exact(4) {
            ...
            if sum < lum_min || sum > lum_max { continue; }
            let mx = r.max(g).max(b) as i32;
            let mn = r.min(g).min(b) as i32;
            if mx - mn < sat_min { continue; }
...
fn extract_app_icon_bmp(exe_path: &str) -> Option<Vec<u8>> {
    ...
    use windows_sys::Win32::UI::WindowsAndMessaging::{DestroyIcon, DrawIconEx, DI_NORMAL, GetSystemMetrics, HICON, SM_CXICON, SM_CYICON};
    ...
    let result = render_to_bgra(width, height, |hdc| {
        unsafe { DrawIconEx(hdc, 0, 0, icon, width, height, 0, std::ptr::null_mut(), DI_NORMAL) != 0 }
    })
    .ok()
...
fn capture_region_bgra(x: i32, y: i32, width: i32, height: i32) -> Result<Vec<u8>, String> {
    render_to_bgra(width, height, |hdc| {
        use windows_sys::Win32::Graphics::Gdi::{BitBlt, CAPTUREBLT, GetDC, ReleaseDC, SRCCOPY};

        let screen = unsafe { GetDC(std::ptr::null_mut()) };
        if screen.is_null() {
            return false;
        }
        let ok = unsafe { BitBlt(hdc, 0, 0, width, height, screen, x, y, SRCCOPY | CAPTUREBLT) != 0 };
        unsafe {
