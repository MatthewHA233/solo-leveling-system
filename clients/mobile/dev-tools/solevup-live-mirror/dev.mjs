...
function start(name, cmd, args, env = {}) {
  ...
  child.on('exit', (code, signal) => {
    ...
    console.log(`[solevup-live-mirror] ${name} exited (${signal || code})`)
    shutdown(code || 1)
  })
  return child
}
...
<html lang="zh-CN">
  <head>
    ...
    <title>Solevup 洪流域 Web Mirror</title>
  </head>
  <body>
    <div id="root"></div>
...
start('api', process.execPath, ['mirror-server.mjs'], {
  SOLEVUP_MIRROR_API_PORT: process.env.SOLEVUP_MIRROR_API_PORT || '8767',
})
...
start('vite', process.platform === 'win32' ? 'npx.cmd' : 'npx', [
  ...
  '0.0.0.0',
  '--port',
  process.env.SOLEVUP_MIRROR_WEB_PORT || '8766',
])
