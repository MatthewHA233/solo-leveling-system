...
DB 同步走 `adb exec-out run-as <pkg> cat databases/perception.db`，**要求 app 可 run-as（debuggable）**：
...
| POST | `/api/sync` | 拉一次手机 `perception.db` 快照 |
...
```bash
SLS_ADB_SERIAL=192.168.0.102:33067 npm run dev   # 多设备时锁定一台
SLS_MIRROR_WEB_PORT=8776 SLS_MIRROR_API_PORT=8777 npm run dev   # 改端口
SLS_CAPTURE_LIMIT=50000 npm run dev              # 拉取条数上限
SLS_FRAMES_ROOT=/some/dir npm run dev            # 帧根目录（默认 ./frames）
```

## 落盘位置（均在 `.cache/`，已 gitignore）

- `.cache/perception.db` — adb 同步下来的 DB 快照
