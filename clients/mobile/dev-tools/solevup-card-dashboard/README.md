# Solevup 卡片对照 Dashboard

调试洪流域"还原卡片/还原动作"功能的本地 dashboard。
用真实 B 站录屏帧 vs Solevup 截屏并排展示 + 缺陷清单。

## 启动

```bash
cd clients/mobile/dev-tools/solevup-card-dashboard
...
- `index.html` — 单页 dashboard（三栏布局：真实帧 / Solevup 截屏 / 缺陷清单）
- `manifest.json` — 对照单元的描述（看哪段、用哪些帧、哪些 Solevup 截屏、什么缺陷）
- `frames/f####.jpg` — 真实 B 站录屏的关键帧
- `shots/*.png` — Solevup app 截屏（手动 adb 抓 + 用 sips 缩小到 700px）
...
1. 抓 Solevup 截屏：
   ...
   adb -s $DEV exec-out screencap -p > /tmp/solevup.png
   sips -Z 700 /tmp/solevup.png --out shots/card-XXX.png
   ```
...
2. 编辑 `manifest.json`，加一段 `compare_units[]`：
   ```json
   {
     ...
     "solevup_shots": [{ "src": "shots/card-xxx.png", "caption": "vN" }],
     "issues": [
