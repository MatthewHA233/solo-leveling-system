...
class PerceptionModule(private val reactContext: ReactApplicationContext) :
  ...
  fun getClickCounts(promise: Promise) {
    try {
      val (list, total) = SolevupAccessibilityService.snapshotClicks()
  ...
  fun resetClickCounts(promise: Promise) {
    try {
      SolevupAccessibilityService.resetClicks()
  ...
  fun isAccessibilityEnabled(promise: Promise) {
    try {
      promise.resolve(SolevupAccessibilityService.isEnabled(reactContext))
    } catch (e: Throwable) {
  ...
  fun openAccessibilitySettings(promise: Promise) {
    try {
      promise.resolve(SolevupAccessibilityService.openSettings(reactContext))
    } catch (e: Throwable) {
