...
### P0 — 主动询问与窗口记忆
[详见设计文档](AI系统/主动询问与窗口记忆.md)：
- [ ] 新窗口无映射 → 主动询问主人在做什么，创建主线/步骤
- [ ] 主人主动说明当前窗口意图
- [ ] 每 3 小时定时刷新已有窗口映射
- [ ] 长时间不操作 → 询问在别处做什么
- [ ] 窗口-任务映射数据库（习惯记录 + 持久化）
- [ ] 窗口记忆作为强上下文注入 Phase 1/2 提示词
...
### P1 — 动机系统 + 战略 Buff 库
[动机系统](面向未来升级/动机系统.md) | [战略 Buff 库](面向未来升级/战略Buff库.md)：
- [ ] 多维度动机定义（有魅力、有钱、技能出众……）
- [ ] 各动机独立经验值 + 等级
- [ ] AI 自动推断活动对应哪个动机
- [ ] 悬浮面板多维度辉光进度条
- [ ] 战略 Buff 库（行动纲领 / 思维框架 / 领域知识 / 成长假设）
- [ ] AI 从对话中自动提炼 Buff 草稿
- [ ] `/buff` 命令管理知识库
- [ ] Buff 库注入 ContextAdvisor，影响 AI 决策
